...
Issues, specs, and tickets for this repo live in the **shared tracker repo `zhaochy1990/stride-devops`** — NOT in this repo's own GitHub Issues. This repo's `git remote` points at `zhaochy1990/auth`, so `gh` cannot infer the target. Point every `gh` call at the shared repo: export `GH_REPO=zhaochy1990/stride-devops` for the session, or pass `-R zhaochy1990/stride-devops` on every command. Do not rely on `gh repo set-default` alone.
...
- **This project's label**: `project:auth`
- **Create an issue**: `gh issue create -R zhaochy1990/stride-devops --label "project:auth" --title "..." --body "..."`. Use a heredoc for multi-line bodies.
- **List this project's issues**: `gh issue list -R zhaochy1990/stride-devops --label "project:auth" --state open --json number,title,body,labels,comments --jq '[.[] | {number, title, body, labels: [.labels[].name], comments: [.comments[].body]}]'` with appropriate `--label` and `--state` filters.
...
Create a GitHub issue in `zhaochy1990/stride-devops` with the `project:auth` label.
...
Used by `/wayfinder`. The **map** is a single issue with **child** issues as tickets. Every map and child also carries the `project:auth` label.

- **Map**: a single issue labelled `wayfinder:map` **and `project:auth`**, holding the Notes / Decisions-so-far / Fog body. `gh issue create -R zhaochy1990/stride-devops --label wayfinder:map --label project:auth`.
- **Child ticket**: an issue linked to the map as a GitHub sub-issue (`gh api` on the sub-issues endpoint). Where sub-issues aren't enabled, add the child to a task list in the map body and put `Part of #<map>` at the top of the child body. Labels: `wayfinder:<type>` (`research`/`prototype`/`grilling`/`task`) **plus `project:auth`**. Once claimed, the ticket is assigned to the driving dev.
...
- **Frontier query**: list the map's open children scoped to this project (`gh issue list -R zhaochy1990/stride-devops --label "project:auth" --state open`, then keep the map's sub-issues / task list), drop any with an open blocker (`issue_dependencies_summary.blocked_by > 0`, or an open issue in the `Blocked by` line) or an assignee; first in map order wins.
