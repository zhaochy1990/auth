...
export default function InviteCodeListPage() {
  ...
  const [grantTestingType, setGrantTestingType] = useState(false);
  ...
  const handleCreate = () => {
    ...
    createMutation.mutate({
      ...
      grants_user_type: grantTestingType ? 'testing' : undefined,
    });
  };
  ...
  const renderUserTypeGrant = (item: InviteCode) =>
    item.grants_user_type ? (
      <Badge variant="blue">{t(`userType.${item.grants_user_type}`)}</Badge>
    ) : (
      <span className="text-gray-400">-</span>
    );
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
          ...
          <label className="flex items-center gap-2 rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700">
            <input
              type="checkbox"
              checked={grantTestingType}
              onChange={(e) => setGrantTestingType(e.target.checked)}
              className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <span>{t('create.testingUserLabel')}</span>
          </label>
          <button
            ...
            <div key={item.id} className="rounded-lg bg-white p-4 shadow-sm ring-1 ring-gray-200">
              ...
              <dl className="mt-3 grid grid-cols-1 gap-3 text-sm">
                ...
                <div>
                  ...
                  <dd className="mt-1">{renderUserTypeGrant(item)}</dd>
                </div>
                <div>
            ...
            {(data || []).length === 0 ? (
              ...
              (data || []).map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">{renderUserTypeGrant(item)}</td>
